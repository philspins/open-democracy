...
| `TRUST_PROXY` | Yes when behind ALB/reverse proxy | server | Set `true` when running behind a trusted reverse proxy (e.g. AWS ALB). Enables: reading the real client IP from `X-Forwarded-For`/`X-Real-IP`; HTTP→HTTPS redirect when `OAUTH_BASE_URL` is `https://`; HSTS header on HTTPS responses |
